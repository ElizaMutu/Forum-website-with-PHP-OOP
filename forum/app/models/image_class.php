...
class Image_class {
    ...
    function resize_image_crop($original_image_path, $cropped_image_path, $width, $height, $where="mid") {
        //detrmine the file type of the original image
        ...
        if(($w == $width) && ($h == $height)) { //no resizing needed. retain dimensions but still create new image
            ...
            //if that created an image smaller that we wanted, try the other way
            if($new_h < $height) {
                $ratio = $height/$w;
                $new_h = $height;
                $new_w = $w * $ratio;
        ...
        if(($new_h != $height) || ($new_w != $width)) {
            ...
            if($new_w > $height) { //crop vertically
                $extra = $new_h - $height;
                $x = 0; //source x
    ...
    function resize_image_max($original_image_path,$resized_image_path,$max_width,$max_height) {
        ...
        //get orientation information if a file is a jpeg
        $orientation = 0;
